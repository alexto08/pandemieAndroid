...
{
    public class TipRestService : TipIRestService
    {
        HttpClient client;

        //se va modifica ulterior cu ip-ul si portul corespunzator
        string RestUrl = "https://192.168.1.203:45457/api/tips/{0}";
        public List<Tip> Items { get; private set; }
        public TipRestService()
        {
        ...
        public async Task<List<Tip>> RefreshDataAsync()
        {
            Items = new List<Tip>();
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Items = JsonConvert.DeserializeObject<List<Tip>>(content);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task SaveTipAsync(Tip item, bool isNewItem = true)
        {
            Uri uri = new Uri(string.Format(RestUrl, string.Empty));
            try
            {
